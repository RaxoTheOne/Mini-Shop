...
@section('title', 'Produkte | Mini-Shop')
                    ...
                    @foreach ($products as $product)
                        <article class="bg-white shadow rounded overflow-hidden hover:shadow-lg transition-shadow">
                            ...
                            <div class="w-full h-48 bg-gray-100 overflow-hidden">
                                ...
                                @else
                                    <a href="{{ route('products.show', $product->slug) }}" class="w-full h-full flex items-center justify-center bg-linear-to-br from-gray-100 to-gray-200">
                            ...
                            <div class="border-t px-4 py-2 flex justify-between items-center bg-gray-50">
                                <form action="{{ route('cart.add', $product->id) }}" method="POST" class="cart-form">
                                    @csrf
                                    <button type="submit"
                                        class="bg-indigo-600 text-white text-sm px-4 py-2 rounded hover:bg-indigo-700 disabled:bg-gray-400 disabled:cursor-not-allowed transition-colors"
                                        @if($product->stock <= 0) disabled @endif>
                                        <span class="button-text">In den Warenkorb</span>
                                        <span class="button-loading hidden">Wird hinzugefügt...</span>
                                    </button>
                                </form>
